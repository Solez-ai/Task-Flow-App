      ...
      <div className="flex justify-between items-center p-2 bg-gray-50 rounded-md dark:bg-slate-800">
        <div className="flex items-center gap-2">
          <Button 
            ...
            className="h-8 flex items-center gap-1 border-task hover:border-task-dark dark:border-task dark:bg-slate-800"
          >
          ...
          {selectedTasks.length > 0 && (
            <Badge variant="secondary" className="bg-task text-white">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  variant="default" 
                  size="sm" 
                  className="h-8 bg-task hover:bg-task-dark dark:bg-task dark:hover:bg-task-dark"
                >
              ...
              <DropdownMenuContent align="end" className="dark:bg-slate-900 dark:border-slate-800">
                <DropdownMenuItem 
                  onClick={() => downloadTasks(true)} 
                  className="dark:text-gray-300 dark:focus:text-white dark:focus:bg-slate-800"
                >
                  Download All Tasks
                </DropdownMenuItem>
                <DropdownMenuItem 
                  onClick={() => downloadTasks(false)} 
                  disabled={selectedTasks.length === 0} 
                  className={`${selectedTasks.length === 0 ? 'opacity-50 cursor-not-allowed' : ''} dark:text-gray-300 dark:focus:text-white dark:focus:bg-slate-800`}
                >
        ...
        {tasks.map(task => (
          <div key={task.id} className="flex items-center">
            <input
              ...
              className="mr-2 h-4 w-4 rounded border-gray-300 text-task focus:ring-task dark:border-slate-600 dark:bg-slate-800"
            />

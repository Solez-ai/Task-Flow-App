...
const BadgeDisplay: React.FC<BadgeDisplayProps> = ({
  ...
  const renderBadgeCategory = (title: string, badges: Badge[]) => {
    ...
    return <div className={compact ? "mb-3" : "mb-6"}>
        <h3 className={compact ? "text-sm font-semibold mb-2" : "text-lg font-semibold mb-3"}>{title}</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {badges.map(badge => <div key={badge.id} className={`flex items-center gap-2 p-2 rounded-md bg-amber-50 border border-amber-100 dark:bg-amber-900/20 dark:border-amber-800/30 ${compact ? 'text-xs' : ''}`}>
  ...
  return <Card className={className}>
      ...
      <CardContent className={compact ? "pt-0" : ""}>
        <ScrollArea className={compact ? "h-[200px] pr-4" : "h-[300px] pr-4"}>
          ...
          {streakBadges.length > 0 && (timeBadges.length > 0 || taskBadges.length > 0 || goalBadges.length > 0) && <Separator className="my-4" />}
          
          {renderBadgeCategory("Time Badges", timeBadges)}
          {timeBadges.length > 0 && (taskBadges.length > 0 || goalBadges.length > 0) && <Separator className="my-4" />}
          
          {renderBadgeCategory("Task Badges", taskBadges)}
          {taskBadges.length > 0 && goalBadges.length > 0 && <Separator className="my-4" />}
